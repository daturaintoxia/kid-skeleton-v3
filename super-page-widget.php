...
class mycustom_page_widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $thumbnail != "none" && $thumbposition == "between" ) {
		?> <a href="<?php echo $permalink;?>"><?php echo get_the_post_thumbnail( $mypageid, $thumbnail );?></a><br /> 
		<?php }
		...
		elseif ( $mycontentformat == 'excerpt' ) {
			...
			if (strlen($content) > 5 && !$forcecfexcerpt){ 				
				...
				?>
		<a class="more" href="<?php if (!$morelink) { echo $permalink; } else { echo $morelink; }?>"><?php $more=get_post_meta($mypageid, 'custommoretext', true);echo apply_filters('post_meta',$more, $mypageid, 'custommoretext', 'rich', true);?></a><?php
		} 
		...
		echo $content;?>
		<a class="more" href="<?php if (!isset ($morelink)) { echo $permalink; } else { ;  echo $morelink; }?>"><?php $more=get_post_meta($mypageid, 'custommoretext', true);echo apply_filters('post_meta',$more, $mypageid, 'custommoretext', 'rich', true);?></a><?php
		} 
	...
	function form( $instance ) {
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id( 'thumbposition' ); ?>" name="<?php echo $this->get_field_name( 'thumbposition' ); ?>" class="widefat" style="width:100%;">
				...
				<option <?php if ( 'between' == $instance['thumbposition'] ) echo 'selected="selected"'; ?>>between</option>
